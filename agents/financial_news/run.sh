...
eval "$("$REPO_ROOT/tools/load_env.sh" GROQ_API_KEY NEWSAPI_KEY GNEWS_KEY SERVERCHAN_KEY)"

SLOT="${1:-unknown}"
shift
LOGFILE="$SCRIPT_DIR/financial_news.log"
...
cd "$SCRIPT_DIR"
"$PYTHON" financial_news.py "$@" >> "$LOGFILE" 2>&1
EXIT_CODE=$?
